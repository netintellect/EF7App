...
namespace ServiceCruiser.Model.Entities.Core.Utilities.TimePeriod
{
	...
	public class TimeRange : ITimeRange
	{
		...
		{
		    if (!end.HasValue) end = TimeSpec.MaxPeriodDate;
			if ( start <= end )
			{
				this.start = start;
			...
			{
				this.end = start;
				this.start = (DateTime)end;
			}
			this.isReadOnly = isReadOnly;
		} // TimeRange

        public TimeRange(TimeSpan start, TimeSpan end) : this(DateTime.Today.Add(start), DateTime.Today.Add(end))
	    {}

	    public TimeRange(DateTime day, TimeSpan start, TimeSpan end) : this(day.Add(start), day.Add(end))
        {}
